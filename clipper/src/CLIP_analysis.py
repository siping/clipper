...
def get_genomic_regions(db):
    ...
    try:
       return { "five_prime_utrs" : pybedtools.BedTool("five_prime_utrs.bed"),
             "three_prime_utrs" : pybedtools.BedTool("three_prime_utrs.bed"),
    ...
    exons = pybedtools.BedTool(map(to_bed, exons)).saveas(os.path.join("exons.bed"))
    
    introns = get_introns(exons).saveas(os.path.join("introns.bed"))
        
    return { "five_prime_utrs" : pybedtools.BedTool(map(to_bed, five_prime_utrs)).saveas(os.path.join("five_prime_utrs.bed")),
             "three_prime_utrs" : pybedtools.BedTool(map(to_bed, three_prime_utrs)).saveas(os.path.join( "three_prime_utrs.bed")),
             "cds" : pybedtools.BedTool(map(to_bed, cds)).saveas(os.path.join("cds.bed")),
             "exons" : exons,
             "introns" : introns }
    

def get_feature_locations(db):
    ...
    #clipper.data_file(
    try:
       return { "five_prime_ends" : pybedtools.BedTool("five_prime_ends.bed"),
             "three_prime_ends" : pybedtools.BedTool("three_prime_ends.bed"),
             "poly_a_sites" : pybedtools.BedTool("poly_a_sites.bed"),
             "stop_codons" : pybedtools.BedTool("stop_codons.bed"),
             "start_codons" : pybedtools.BedTool("start_codons.bed"),
             "transcription_start_sites" : pybedtools.BedTool("transcription_start_sites.bed")}
    ...
    return { "five_prime_ends" : pybedtools.BedTool(five_prime_ends).saveas(os.path.join( "five_prime_ends.bed")),
             "three_prime_ends" :pybedtools.BedTool(three_prime_ends).saveas(os.path.join("three_prime_ends.bed")),
             "poly_a_sites" :  pybedtools.BedTool(poly_a_sites).saveas(os.path.join("poly_a_sites.bed")),
             "stop_codons" :  pybedtools.BedTool(stop_codons).saveas(os.path.join("stop_codons.bed")),
             "start_codons" :  pybedtools.BedTool(start_codons).saveas(os.path.join( "start_codons.bed")),
             "transcription_start_sites" : pybedtools.BedTool(transcription_start_sites).saveas(os.path.join("transcription_start_sites.bed"))}
...
def calculate_peak_locations(bedtool, genes, db):
    ...
    regions = get_genomic_regions(db)
    print "Done"
    ...
    features = get_feature_locations(db)
    print "done"
...
def normalize_wiggle(bedtool, bw_pos, bw_neg):
    bed_center = bedtool.each(small_peaks).saveas()
    ...
    return bedtool_df_mag_normalized
...
def main(options):
    ...
    types, premRNA_positions, mRNA_positions, exon_positions, intron_positions, features_transcript_closest, features_mrna_closest, distributions = calculate_peak_locations(cluster_regions['all']['real'], genes_dict, options.db)
